...
- Always use the `user.name` and `user.email` already configured in the repository's git config. Do **not** hardcode or override credentials.
- Never add AI-tool attribution to commit messages (no "Co-authored-by: Antigravity", "Generated by …", "Codex", or similar lines).
